...
fighter=0
def MoveBW():
    global xMove, yMove, BW, RW, BWMove, RWMove, RangeW
...
def MoveBA():
    global xMove, yMove, BA, RA, BAMove, RAMove, RangeA
    #сначала ходят все синие, потом все красные 
    i=0
    while i<len(BA):
        x1=BA[i]
...
while True:
    ...
    lenBW=str(int(len(BW)/2))
    lenRW=str(int(len(RW)/2))
    ...
    sc.blit(text1, (690, 10))
    sc.blit(text2, (670, 100))
    sc.blit(text1b, (710, 50))
    sc.blit(text2r, (710, 160))
    sc.blit(W, (8, 40))
    sc.blit(A, (88, 40))
    ...
    i=0
    while i<len(BAMove):
        pygame.draw.rect(sc, (BLUE), (BAMove[i]-razmer/2, BAMove[i+1]-razmer/2, razmer, razmer))
        #if p==2:
            #BW=BWMove
            #p=1
        i=i+2
    ...
    i=0
    while i<len(RAMove):
        pygame.draw.rect(sc, (RED), (RAMove[i]-razmer/2, RAMove[i+1]-razmer/2, razmer, razmer))
        #if p==1:
            #RW=RWMove
            #p=0
        i=i+2
