#v.005
# добавлены присты-хиллеры, испарвлен баг с движением по оси y, добавлена музыка
#
#
...
def MoveBP():
    ...
    while i<len(BP):
        ...
        while j<len(BA):
            ...
            j=j+2
        j=0
        if mas!='BW' and mas!='BA' and mas!='BSH':
            while j<len(BP):
                x=BP[j]
                y=BP[j+1]
                if x1!=x and y1!=y:
        ...
        if (x2-x1)!=0:
            ...
            sinc=math.radians(90)
            sinc=math.sin(sinc)
            if Distance>RangeP+RangeW/2:
                c=StepP
            if Distance-StepP<=RangeP+RangeW/2:
                c=Distance-RangeP
            if Distance<=RangeP+RangeW/2:
                c=0
            a=(c/sinc)*sina
...
def MoveRP():
    ...
    while i<len(RP):
        ...
        while j<len(RA):
            ...
            j=j+2
        j=0
        if mas!='RW' and mas!='RA' and mas!='RSH':
            while j<len(RP):
                x=RP[j]
                y=RP[j+1]
                if x1!=x and y1!=y:
...
def CheckWinner():
    ...
    if len(RW)+len(RA)+len(RSH)+len(RP)==0 and len(BW)+len(BA)+len(BSH)+len(BP)==0:
        mes=('tie!')
        clr=(100, 100, 100)
        gameover=True
    elif len(BW)+len(BA)+len(BSH)+len(BP)==0:
        mes=('RED wins!')
        clr=RED
        sound5.play()
        gameover=True
    elif len(RW)+len(RA)+len(RSH)+len(RP)==0:
        mes=('BLUE wins!')
        clr=BLUE
        sound3.play()
        gameover=True
    elif len(BW)+len(BA)+len(BSH)==0 and len(RW)+len(RA)+len(RSH)==0:
        if len(BP)>len(RP):
            mes=('BLUE wins!')
            clr=BLUE
            sound3.play()
...
while True:
    ...
    lenBW=str(int(len(BW)/2)+int(len(BA)/2)+int(len(BSH)/2)+int(len(BP)/2))
    lenRW=str(int(len(RW)/2)+int(len(RA)/2)+int(len(RSH)/2)+int(len(RP)/2))
