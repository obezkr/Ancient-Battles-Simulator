    ...
    for i in pygame.event.get():
        ...
        if i.type == pygame.KEYUP:
            ...
            if i.key == pygame.K_r:
                ...
                RW = data.decode("utf-8")
                RW=RW.replace('[', '')
                RW=RW.replace(']', '')
                RW=RW.split(', ')
                if len(RW)>1:
                    for i1 in range(0, len(RW), 2):
                        RW[i1]=float(RW[i1])
                        RW[i1+1]=float(RW[i1+1])
                        RWHP.append(WHP)
                ...
                # принимаем RA
                data = conn.recv(1024*256)
                RA = data.decode("utf-8")
                RA=RA.replace('[', '')
                RA=RA.replace(']', '')
                RA=RA.split(', ')
                if len(RA)>1:
                    for i1 in range(0, len(RA), 2):
                        RA[i1]=float(RA[i1])
                        RA[i1+1]=float(RA[i1+1])
                        RAHP.append(AHP)
                ...
                # принимаем RSH
                data = conn.recv(1024*256)
                RSH = data.decode("utf-8")
                RSH=RSH.replace('[', '')
                RSH=RSH.replace(']', '')
                RSH=RSH.split(', ')
                if len(RSH)>1:
                    for i1 in range(0, len(RSH), 2):
                        RSH[i1]=float(RSH[i1])
                        RSH[i1+1]=float(RSH[i1+1])
                        RSHHP.append(SHHP)
                ...
                RP=RP.split(', ')
                if len(RP)>1:
                    for i1 in range(0, len(RP), 2):
                        RP[i1]=float(RP[i1])
                        RP[i1+1]=float(RP[i1+1])
                        RPHP.append(PHP)
                ...
                RC=RC.split(', ')
                if len(RC)>1:
                    for i1 in range(0, len(RC), 2):
                        RC[i1]=float(RC[i1])
                        RC[i1+1]=float(RC[i1+1])
                        RCHP.append(CHP)
